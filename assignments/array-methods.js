...
let fullName = runners.forEach(function(participant) {
...
let allCaps = runners.map(function(nameCaps){
    return (nameCaps.first_name.toUpperCase());
});
console.log(allCaps); 
...
let alphaLast = runners.map(function(lastName){
    return (lastName.last_name);
});
console.log(alphaLast.sort());


// Problem 2
// Everyone that donated < $100
let largeDonations = runners.filter(function(donat){
    return donat.donation > 100;
});
console.log(largeDonations);

// Problem 3
//Everybodys ID's added
let idAdded = runners.reduce(function(ar, currentVal){
    return ar + currentVal.id;
}, 0);
console.log(idAdded);